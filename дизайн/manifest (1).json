(() => {
  if (document.getElementById('ekt-design-preview-host')) return;
  const isExtension = Boolean(globalThis.chrome?.runtime?.getURL);
  // Local preview uses the exact same window and pointer handling.
  if (!isExtension && location.origin !== 'http://127.0.0.1:8788') return;
  const appURL = isExtension ? chrome.runtime.getURL('app.html') : new URL('extension/app.html', document.baseURI).href;
  const ownOrigin = isExtension ? chrome.runtime.getURL('').replace(/\/$/, '') : location.origin;
  const host = document.createElement('div'); host.id = 'ekt-design-preview-host';
  const root = host.attachShadow({ mode: 'open' });
  const style = document.createElement('style');
  style.textContent = `
    :host{all:initial;position:fixed;inset:0;pointer-events:none;z-index:2147483647;font-family:Arial,sans-serif}
  ...
  const panel=el('section','panel');panel.hidden=true;panel.setAttribute('aria-label','Макет консультанта EKT');
  ...
  const frame=el('iframe');frame.src=appURL;frame.title='Дизайн-макет консультанта EKT';
  ...
  window.addEventListener('resize',layout);window.addEventListener('message',event=>{if(event.source===frame.contentWindow&&event.origin===ownOrigin&&event.data?.type==='ekt-design-close')setOpen(false);});
  if(isExtension)chrome.runtime.onMessage.addListener((message,sender,respond)=>{if(sender.id===chrome.runtime.id&&message?.type==='ekt-design-open'){setOpen(true);respond({opened:true});}});
